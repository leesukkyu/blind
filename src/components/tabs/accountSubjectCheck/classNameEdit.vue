...
export default {
    ...
    option: {
      ...
      tagScript: {
        ...
        onCreate: function(data) {
          this.$http
            .post("/basic_api/account_classname_reg", {
              name: data.name,
        ...
        onDelete: function(data) {
          console.log("delete...");
          this.$http
            .post("/basic_api/account_classname_del", {
              idx: data.item.idx
            })
            .then(rs => {
              if ((rs.status = "Y")) {
        ...
        onUpdate: function(data) {
          this.$http
            .post("/basic_api/account_classname_edit", {
              idx: data.item.idx,
              name: data.newName
            })
            .then(rs => {
  ...
  beforeMount: function() {
    this.self = this;
    this.option.tagScript.label = 'menu.' + this.tabInfo.item.code + '.child.class_name_edit';
  ...
  methods: {
    onChangeTreeActiveList: function() {
      ...
      if (this.pageStorage.treeActiveItemList.length != 0) {
        ...
        this.$http
          .post("/basic_api/account_classname_call", {
