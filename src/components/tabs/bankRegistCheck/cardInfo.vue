  ...
  <div>
    <div class="pt-2 background">
      <v-toolbar flat color="background">
        <v-toolbar-title class="pr-3">{{ $t('menu.' + tabInfo.item.code + ".child.card_info") }}</v-toolbar-title>
              ...
              <v-layout wrap>
                <v-flex xs12 sm6 md6>
                  <v-text-field
                    ...
                    :label="$t('menu.' + tabInfo.item.code + '.child.card_name')"
                  ></v-text-field>
                </v-flex>
                <v-flex xs12 sm6 md6>
                  <v-text-field
...
export default {
  ...
  methods: {
    ...
    deleteItem() {
      ...
      this.$http
        .post("basic_api/bank_regist_card_del", {
    ...
    save() {
      // 새로운 저장
      this.$http
        .post("/basic_api/bank_regist_card_regist", {
          userid: this.$store.getters.getLoginInfo.loginInfo.id,
    ...
    update() {
      // 업데이트
      this.$http
        .post("/basic_api/bank_regist_card_edit", {
          userid: this.$store.getters.getLoginInfo.loginInfo.id,
