...
export default {
    ...
    option: {
      ...
      tagScriptCode: {
        ...
        onCreate: function(data) {
          console.log("생성");
          this.$http
            .post("/goods_api/wondan_color_code", {
            ...
            .then(rs => {
              if (rs.status == "N") {
                this.$http
                  .post("/goods_api/goods_wondan_color_reg", {
                    userid: this.$store.getters.getLoginInfo.loginInfo.id,
        ...
        onDelete: function(data) {
          console.log("delete...");
          this.$http
            .post("/goods_api/goods_wondan_color_del", {
              userid: this.$store.getters.getLoginInfo.loginInfo.id,
              idx: data.item.idx
            })
            .then(rs => {
        ...
        update: function(data) {
          ...
          this.$http
            .post("/goods_api/goods_wondan_color_edit", {
              userid: this.$store.getters.getLoginInfo.loginInfo.id,
              idx: data.item.idx,
              name: data.newName,
              code: data.newCode
  ...
  beforeMount: function() {
    ...
    this.option.tagScriptCode.label =
      "menu." + this.tabInfo.item.code + ".child.color_name_edit";
  ...
  methods: {
    ...
    loadFabricNameList: function() {
      this.$http
        .post("/goods_api/goods_wondan_color_list", {
          userid: this.$store.getters.getLoginInfo.loginInfo.id,
          idx: this.pageStorage.productTreeActiveItemList[0].idx
        })
        .then(rs => {
