...
export default {
    ...
    option: {
      ...
      tagScript: {
        ...
        onCreate: function(data) {
          this.$http
            .post("/goods_api/goods_blind_fict_width_reg", {
        ...
        onDelete: function(data) {
          console.log("delete...");
          this.$http
            .post("/goods_api/goods_blind_fict_width_del", {
              userid: this.$store.getters.getLoginInfo.loginInfo.id,
              idx: data.item.idx
            })
            .then(rs => {
        ...
        onUpdate: function(data) {
          this.$http
            .post("/goods_api/goods_blind_fict_width_edit", {
              userid: this.$store.getters.getLoginInfo.loginInfo.id,
              idx: data.item.idx,
              name: data.newName
            })
  ...
  methods: {
    ...
    loadWidthSetting: function() {
      ...
      this.$http
        .post("/goods_api/goods_blind_fict_width_list", {
          userid: this.$store.getters.getLoginInfo.loginInfo.id,
          idx: this.pageStorage.productTreeActiveItemList[0].idx
        })
        .then(rs => {
