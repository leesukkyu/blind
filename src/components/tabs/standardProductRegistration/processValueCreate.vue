        ...
        <v-layout wrap>
          <v-flex xs12 mt-2>
            ...
            <v-btn-toggle :mandatory="true" v-model="editedItem.val1">
              <v-btn value="선염" color="green">{{ $t('menu.' + tabInfo.item.code + '.child.pre_dyeing') }}</v-btn>
              <v-btn value="후염" color="green">{{ $t('menu.' + tabInfo.item.code + '.child.after_dyeing') }}</v-btn>
              <v-btn value="천연" color="green">{{ $t('menu.' + tabInfo.item.code + '.child.natural') }}</v-btn>
            </v-btn-toggle>

            <!-- 방염 -->
            <span class="px-4 body-2">{{ $t('menu.' + tabInfo.item.code + '.child.flame_retardation') }}</span>
            <v-btn-toggle :mandatory="true" v-model="editedItem.val2">
              ...
              <v-btn value="난연" color="green">{{ $t('menu.' + tabInfo.item.code + '.child.flame_retardant') }}</v-btn>
              <v-btn value="필무" color="green">{{ $t('menu.' + tabInfo.item.code + '.child.no_require') }}</v-btn>
            </v-btn-toggle>
          </v-flex>
          ...
          <v-flex xs12 mt-3>
            ...
            <v-btn-toggle :mandatory="true" v-model="editedItem.val3">
              <v-btn value="안 됨" color="green">{{ $t('menu.' + tabInfo.item.code + '.child.no') }}</v-btn>
              <v-btn value="OK" color="green">OK</v-btn>
              <v-btn value="세미암막" color="green">{{ $t('menu.' + tabInfo.item.code + '.child.semi_dark') }}</v-btn>
              <v-btn value="DimOut" color="green">DimOut
                <v-text-field
              ...
              <v-btn value="BlackOut" color="green">BlackOut
                <v-text-field
...
export default {
  ...
  methods: {
    ...
    save: function() {
      ...
      this.$http
        .post("/goods_api/goods_processval_reg", rq)
        .then(rs => {
          console.log(rs);
