...
export default {
    ...
    option: {
      ...
      tagScript: {
        ...
        onCreate: function(data) {
          console.log('생성생성');
          this.$http
            .post("/goods_api/goods_blind_category_add", {
        ...
        onDelete: function(data) {
          console.log("delete...");
          this.$http
            .post("/goods_api/goods_blind_category_del", {
              userid: this.$store.getters.getLoginInfo.loginInfo.id,
              idx: data.item.idx
            })
            .then(rs => {
        ...
        onUpdate: function(data) {
          this.$http
            .post("/goods_api/goods_blind_category_edit", {
              userid: this.$store.getters.getLoginInfo.loginInfo.id,
              idx: data.item.idx,
              gc_name: data.newName
            })
            .then(rs => {
              if ((rs.status = "Y")) {
                this.eventBus.$emit("requestProductTreeLoad");
  ...
  methods: {
    ...
    loadWidthSetting: function() {
      ...
      this.$http
        .post("/goods_api/goods_blind_category_list", {
          userid: this.$store.getters.getLoginInfo.loginInfo.id,
